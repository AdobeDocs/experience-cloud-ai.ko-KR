---
title: Adobe CX 엔터프라이즈 MCP 서버
description: Adobe CX Enterprise MCP 서버는 Adobe CX Enterprise용 통합 MCP로서, MCP 클라이언트에게 지원되는 제품 도구에 대한 단일 연결을 제공합니다.
...
# Adobe CX 엔터프라이즈 MCP 서버 {#mcp-overview}

Adobe CX Enterprise MCP 서버는 Adobe CX Enterprise용 MCP(Unified Model Context Protocol)입니다. 하나의 연결로 MCP 호환 클라이언트는 조직 및 계정이 사용할 수 있는 Adobe 제품 도구에 액세스할 수 있습니다.

>[!IMPORTANT]
>
>CX 엔터프라이즈 MCP 도구를 사용하려면 먼저 Adobe 조직이 활성화되어야 합니다. 조직에 아직 액세스 권한이 없는 경우 Adobe 계정 팀에 연락하여 조직에 대한 활성화를 요청하십시오.

모든 MCP 클라이언트 설정에 CX 엔터프라이즈 엔드포인트 사용:
...
CX Enterprise 는 CX Enterprise 제품 툴의 관리 엔드포인트를 제공합니다. 별도의 제품 서버를 추가하는 대신 엔드포인트에 한 번 연결하고 권한이 부여된 솔루션에 표시된 제품 도구를 사용합니다.
...
>[!NOTE]
>
>도구 가용성은 제품 라이선스, 조직 활성화, 제품 권한 및 인증에 사용되는 Adobe 자격 증명에 따라 다릅니다. MCP는 조직과 사용자 계정이 액세스할 수 있는 도구만 표시합니다. [CX 엔터프라이즈 MCP 도구 액세스](access.md)를 참조하십시오.



## 시작하기 {#mcp-get-started}

1. [CX 엔터프라이즈 MCP 도구에 액세스](access.md)를 검토하여 제품의 가용성, 사용 여부 및 사용 권한을 확인하십시오.
2. [CX Enterprise MCP용 Adobe 설치](install.md)를 따라 MCP 클라이언트를 끝점에 연결합니다.
